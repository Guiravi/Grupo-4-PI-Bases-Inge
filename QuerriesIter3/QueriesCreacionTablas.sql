...
(
	...
	CONSTRAINT FK_Miembro_CatalogoPais FOREIGN KEY (paisFK) REFERENCES Catalogo.Pais(paisPK)
		ON DELETE SET DEFAULT --A nivel de opciones para el usuario no se va a mostrar el valor N/A, pero si el país escogido llegara a borrarse se quiere representar esta falta de país con el valor N/A
		ON UPDATE CASCADE 
);
...
(
	...
	CONSTRAINT FK_MiembroCalificaArticulo_Articulo FOREIGN KEY (idArticuloFK) REFERENCES Articulo(articuloAID)
		ON DELETE CASCADE --Se quieren eliminar los registro de calificaciones para el artículo borrado. No se quieren mantener calificaciones sin articulo.
		ON UPDATE CASCADE
);
...
(
	...
	CONSTRAINT FK_NucleoRevisaArticulo_Miembro FOREIGN KEY (usernameMiemFK) REFERENCES Miembro(usernamePK)
		ON DELETE NO ACTION --Se quiere mantener el registro del puntaje y comentarios dados por este autor, además de que esto influye en el cálculo de los méritos.
		ON UPDATE CASCADE,
	CONSTRAINT FK_NucleoRevisaArticulo_Articulo FOREIGN KEY (idArticuloFK) REFERENCES Articulo(articuloAID)
		ON DELETE CASCADE --Si se elimina el artículo en su proceso de revisión se quieren eliminar todos los registros de revisión asociados a este artículo. No se quieren registros de revision sin un articulo asociado.
		ON UPDATE CASCADE,
...
(
	...
	CONSTRAINT FK_ArticuloTrataTopico_CategoriaTopico FOREIGN KEY (nombreCategoriaFK, nombreTopicoFK) REFERENCES CategoriaTopico(nombreCategoriaPK, nombreTopicoPK)
		ON DELETE NO ACTION --Se tienen que borrar todos los artículos asociados al tópico antes de borrar el tópico pues no se pueden tener articulos que no esten asociados a un topico.
		ON UPDATE CASCADE,
	CONSTRAINT FK_ArticuloTrataTopico_Articulo FOREIGN KEY (idArticuloFK) REFERENCES Articulo(articuloAID)
		ON DELETE CASCADE --Se desea eliminar la tupla que asociaba al artículo borrado con un tópico pues no se quiere mantener datos sobre un articulo borrado.
		ON UPDATE CASCADE
);
...
(
	...
	CONSTRAINT FK_MiembroPasatiempo_Miembro FOREIGN KEY (usernameFK) REFERENCES Miembro(usernamePK)
		ON DELETE CASCADE --Si se elimina al miembro, se quiere eliminar la relación que tenía este miembro con sus pasatiempos. No se quiere almacenar informacion sobre un miembro borrado.
		ON UPDATE CASCADE,
);
...
(
	...
	CONSTRAINT FK_MiembroIdioma_Miembro FOREIGN KEY (usernameFK) REFERENCES Miembro(usernamePK)
		ON DELETE CASCADE --Si se elimina al miembro, se quiere eliminar la relación que tenía este miembro con sus idiomas. No se quiere almacenar informacion sobre un miembro borrado.
		ON UPDATE CASCADE,
);
...
(
	...
	CONSTRAINT FK_MiembroHabilidad_Miembro FOREIGN KEY (usernameFK) REFERENCES Miembro(usernamePK)
		ON DELETE CASCADE --Si se elimina al miembro, se quiere eliminar la relación que tenía este miembro con sus habilidades. No se quiere almacenar informacion sobre un miembro borrado.
		ON UPDATE CASCADE,
);
